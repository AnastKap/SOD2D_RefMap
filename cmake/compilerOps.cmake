...
if(CMAKE_C_COMPILER_ID STREQUAL "GNU")
	...
	set(CMAKE_Fortran_FLAGS_DEBUG ${CMAKE_Fortran_FLAGS_DEBUG} "-Wall -Wextra -Wpedantic -Wconversion-extra")
...
elseif(CMAKE_C_COMPILER_ID STREQUAL "NVHPC" OR CMAKE_C_COMPILER_ID STREQUAL "PGI")
	...
	if(STORE_INTERMEDIATE_FILES)
		set(CMAKE_C_FLAGS ${CMAKE_C_FLAGS} "-Mkeepasm -gpu=keep,nodebug")
		set(CMAKE_CXX_FLAGS ${CMAKE_CXX_FLAGS} "-Mkeepasm -gpu=keep,nodebug")
		set(CMAKE_Fortran_FLAGS ${CMAKE_Fortran_FLAGS} "-Mkeepasm -gpu=keep,nodebug")
	endif()
	# Debug
	set(CMAKE_C_FLAGS_DEBUG ${CMAKE_C_FLAGS_DEBUG} "-Minform=inform -C -Mchkptr -traceback -Ktrap=fp,unf")
	set(CMAKE_CXX_FLAGS_DEBUG ${CMAKE_CXX_FLAGS_DEBUG} "-Minform=inform -C -Mchkstk -traceback -Ktrap=fp,unf")
	set(CMAKE_Fortran_FLAGS_DEBUG ${CMAKE_Fortran_FLAGS_DEBUG} "-Minform=inform -C -Mchkstk -traceback -Ktrap=fp,unf")
	# Release
	...
	if(USE_GPU AND USE_MEM_MANAGED)
		...
		if(USE_PCPOWER)
			...
			set(CMAKE_C_FLAGS ${CMAKE_C_FLAGS} "-gpu=cc86,managed -cuda -acc")
			set(CMAKE_CXX_FLAGS ${CMAKE_CXX_FLAGS} "-gpu=cc86,managed -cuda -acc")
			set(CMAKE_Fortran_FLAGS ${CMAKE_Fortran_FLAGS} "-gpu=cc86,managed -cuda -acc")
		endif()
	elseif(USE_GPU)
		...
		if(USE_PCPOWER)
			...
			message("IMPORTANT: Verify the device compute capability!!!")
			set(CMAKE_C_FLAGS ${CMAKE_C_FLAGS} "-gpu=cc61 -cuda -acc")
			set(CMAKE_CXX_FLAGS ${CMAKE_CXX_FLAGS} "-gpu=cc61 -cuda -acc")
			set(CMAKE_Fortran_FLAGS ${CMAKE_Fortran_FLAGS} "-gpu=cc61 -cuda -acc")
		endif()
	else()
...
elseif(CMAKE_C_COMPILER_ID STREQUAL "Clang")
	execute_process(
		COMMAND bash -c "ompi_info | grep \"MPI extensions\" | grep rocm"
		OUTPUT_VARIABLE MPI_WITH_ROCM
		OUTPUT_STRIP_TRAILING_WHITESPACE  # Strip trailing whitespace
	)
	if(NOT MPI_WITH_ROCM)
		...
		message("-- AMD compiler detected")
		# Common NVHPC+MPI flags
		set(CMAKE_C_FLAGS ${CMAKE_C_FLAGS} "-cpp -D_USE_ROCTX")
		set(CMAKE_CXX_FLAGS ${CMAKE_CXX_FLAGS} "-cpp -D_USE_ROCTX")
		set(CMAKE_Fortran_FLAGS ${CMAKE_Fortran_FLAGS} "-cpp -D_USE_ROCTX")
		SET(CMAKE_EXE_LINKER_FLAGS  "${CMAKE_EXE_LINKER_FLAGS} -L/opt/rocm-6.2.1/lib -lamdhip64 -lroctx64")
		if(STORE_INTERMEDIATE_FILES)
			set(CMAKE_C_FLAGS ${CMAKE_C_FLAGS} "--save-temps")
			set(CMAKE_CXX_FLAGS ${CMAKE_CXX_FLAGS} "--save-temps")
			set(CMAKE_Fortran_FLAGS ${CMAKE_Fortran_FLAGS} "--save-temps")
		endif()
		# GPU options
		if(USE_GPU)
			message("Setting up OpenMP flags with managed memory")
			message("IMPORTANT: Verify the device compute capability!!!")
			set(CMAKE_C_FLAGS ${CMAKE_C_FLAGS} "-fopenmp -fopenmp-targets=gfx908 -Xopenmp-target=amdgcn-amd-amdhsa -march=gfx908 DNOACC")
			set(CMAKE_CXX_FLAGS ${CMAKE_CXX_FLAGS} "-fopenmp -fopenmp-targets=gfx908 -Xopenmp-target=amdgcn-amd-amdhsa -march=gfx908 -DNOACC")
			set(CMAKE_Fortran_FLAGS ${CMAKE_Fortran_FLAGS} "-fopenmp -fopenmp-targets=gfx908 -Xopenmp-target=amdgcn-amd-amdhsa -march=gfx908 -DNOACC")
		else()
			#AMD COMPILERS WITHOUT GPUs
			set(CMAKE_C_FLAGS ${CMAKE_C_FLAGS} "-DNOACC -DOPENMP")
			set(CMAKE_CXX_FLAGS ${CMAKE_CXX_FLAGS} "-DNOACC -DOPENMP")
			set(CMAKE_Fortran_FLAGS ${CMAKE_Fortran_FLAGS} "-DNOACC -DOPENMP")
			# Debug
			set(CMAKE_C_FLAGS_DEBUG ${CMAKE_C_FLAGS_DEBUG} "-DNOACC -DOPENMP")
			set(CMAKE_CXX_FLAGS_DEBUG ${CMAKE_CXX_FLAGS_DEBUG} "-DNOACC -DOPENMP")
			set(CMAKE_Fortran_FLAGS_DEBUG ${CMAKE_Fortran_FLAGS_DEBUG} "-DNOACC -DOPENMP")
			# Release
			set(CMAKE_C_FLAGS_RELEASE ${CMAKE_C_FLAGS_RELEASE} "-DNOACC -DOPENMP")
			set(CMAKE_CXX_FLAGS_RELEASE ${CMAKE_CXX_FLAGS_RELEASE} "-DNOACC -DOPENMP")
			set(CMAKE_Fortran_FLAGS_RELEASE ${CMAKE_Fortran_FLAGS_RELEASE} "-DNOACC -DOPENMP")
		endif()
	endif()
else()
